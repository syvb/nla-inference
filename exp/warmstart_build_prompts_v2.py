"""V2: build warm-start prompts from FULL prefix (not 50-char chars_before).

Reconstructs detokenized_text_truncated the same way stage0_extract.py does:
  text = tokenizer.apply_chat_template(messages, tokenize=False)  -> tokenize
  truncated = tokenizer.decode(token_ids[:unpadded_pos+1], skip_special_tokens=True)

Verifies that chars_before (last 50 chars from activations parquet) matches the
last 50 chars of our reconstruction — if not, our reconstruction is wrong.
"""
...
def main():
    ...
    # Build warm-start prompts from the FULL decoded prefix
    prompts = [_DEFAULT_INSTRUCTION.format(text=r["decoded_full"]) for r in out_records]
