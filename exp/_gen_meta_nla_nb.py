...
"!pip install -q -U 'transformers==5.8.1' 'tokenizers==0.22.2' safetensors pyyaml httpx orjson accelerate huggingface_hub hf_transfer",
)
...
"try:",
"    import hf_transfer  # noqa: F401  — fast, resumable Rust downloader",
...
"",
"def fetch(repo, **kw):",
"    # Colab ↔ HF can drop large transfers (the 10.9 GB AR is the usual victim).",
"    # snapshot_download resumes already-downloaded blobs, so just retry on timeout.",
"    for attempt in range(10):",
"        try:",
"            return snapshot_download(repo, **kw)",
"        except Exception as e:",
"            print(f'  download retry {attempt + 1}/10 ({type(e).__name__}) — resuming...')",
"            time.sleep(5)",
"    raise RuntimeError(f'could not download {repo} after 10 attempts')",
"",
"av_dir = fetch(AV_REPO)",
...
"- `'last_prompt'` — the final prompt token. NOTE this is the `\\n` after `<|im_start|>assistant` (the",
"  generation-prompt suffix), so it probes the AV's *about-to-respond* state, not a content token.",
"- `'expl_start'` — the first `<explanation>` content token (after the tag + newline). Content-anchored.",
...
code(
"def explanation_start_pos(res):",
"    # Absolute index into res['hidden'] of the first <explanation> CONTENT token",
"    # (after the opening tag and any newline/whitespace).",
"    pl = res['prompt_len']",
"    new_ids = res['new_ids'].tolist()",
"    full = av.tok.decode(new_ids)",
"    i = full.find('<explanation>')",
"    if i < 0:",
"        return pl                       # no tag (odd/truncated gen) — fall back to first gen token",
"    c = i + len('<explanation>')",
"    while c < len(full) and full[c].isspace():",
"        c += 1                          # skip the newline(s)/spaces after the tag",
"    for k in range(len(new_ids)):       # map char offset -> token index via growing prefix",
"        if len(av.tok.decode(new_ids[:k + 1])) > c:",
"            return pl + k",
"    return pl + len(new_ids) - 1",
"",
...
"    if where == 'expl_start':      return explanation_start_pos(res)   # 1st explanation content token",
